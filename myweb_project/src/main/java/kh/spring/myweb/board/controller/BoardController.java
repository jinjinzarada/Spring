...
public class BoardController {
	...
	public ModelAndView list(@RequestParam(name = "page", defaultValue = "1") int currentPage, ModelAndView mv,
			...
			@RequestParam(name = "test5", defaultValue = "5") int test5, HttpServletRequest req) {
...
//		request.getRequestDispatcher("WEB-INF/view/board/boardlist.jsp").forward(request, response);
	...
	public ModelAndView delete(ModelAndView mv, @RequestParam(name = "bNo", defaultValue = "0") int bNo,
		...
		mv.addObject("test2", bNo); // @RequestParam url에 보임

		rttr.addAttribute("test5", bNo); // @RequestParam url에 안보임
		if (board > 0) {
	...
	public ModelAndView writeWithFileDo(ModelAndView mv,
			@RequestParam(name = "uploadfile", required = false) MultipartFile[] files,
			MultipartHttpServletRequest multiReq, Board board, HttpServletRequest req) {

		System.out.println(multiReq.getParameter("bTitle"));
		System.out.println(req.getParameter("bTitle"));
		System.out.println("======");
		MultipartFile tempFile = multiReq.getFile("uploadfile");
		System.out.println(tempFile);
		System.out.println(files);
		if(files != null) {
			for(int i=0; i<files.length;i++) {
				if(files[i] !=null && !files[i].equals("")) {
					saveFile(files[i], req);
				}
			}
		}
		mv.setViewName("redirect:/board/list");
